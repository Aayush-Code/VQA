...
# print("Initializing Video Quality Assessment:")
...
with tf.device('/CPU:0'):
  ...
  for img_path in imgs:
    ...
    total_rating = total_rating + mean
    count = count + 1

    progress = int((count / total_imgs) * 100)
    avg_rating = total_rating / count
    # print("Rating/%.3f" % avg_rating)
    ...
    # print("NIMA Score : %0.3f +- (%0.3f)" % (mean, std), flush=True)
    # print("\n\n")
  # if rank_images:
  #   print("*" * 5, "Ranking Images", "*" * 5)
  #   score_list = sorted(score_list, key=lambda x: x[1], reverse=True)
  #
  #   for i, (name, score) in enumerate(score_list):
  #     print("%d)" % (i + 1), "%s : Score = %0.5f" % (name, score))
  #   print("\n\nVideo Assessment Score = %0.3f" % avg_rating)
